...
Raw mode streams the payload bytes directly. The bootloader uses UART idle time
to detect the end of the image and release the CPU.
"""
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--raw", action="store_true", help="Send payload bytes only (no length/entry header)")
...
def main() -> int:
    ...
    print(f"[uart-upload] Mode: {'raw payload' if args.raw else 'length+entry header'}")
